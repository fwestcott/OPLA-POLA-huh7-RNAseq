...
Decision_results1 <- decideTests(res1, method="global", adjust.method="BH", p.value=0.05, lfc = 0.1)
Decision_results2 <- decideTests(res2, method="global", adjust.method="BH", p.value=0.05, lfc = 0.1)
Decision_results3 <- decideTests(res3, method="global", adjust.method="BH", p.value=0.05, lfc = 0.1)
...
datatable(diffGenes.avg.df, 
          extensions = c('KeyTable', "FixedHeader"), 
          caption = 'Table 1: DEGs for Control, OPLA and POLA',
...
graph <- diffGenes.avg.df.pivot %>%
  dplyr::filter(grepl('MAPK9', geneID))
...
heatmap.2(selected_genes, 
          ...
          scale='row',
          density.info="none", trace="none",  
          cexRow= 0.9, cexCol=0.9, margins=c(8,20))
...
pathview(gene.data  = fc.vector, pathway.id = "hsa04210",species  = "hsa")
